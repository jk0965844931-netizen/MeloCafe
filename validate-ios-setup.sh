...
# Check 4: Verify GUI stub exists
echo ""
echo "📋 Checking iOS GUI stub..."
if [ -f "src/gui/iosgui/IOSGui.h" ] && [ -f "src/gui/iosgui/IOSGui.cpp" ]; then
    echo -e "${GREEN}✓${NC} iOS GUI stub files exist"
else
    echo -e "${RED}✗${NC} iOS GUI stub files missing"
    ((errors++))
fi

# Check 5: Verify OpenGL compatibility headers
echo ""
echo "📋 Checking OpenGL compatibility headers..."
if [ -f "src/Cafe/HW/Latte/Renderer/OpenGL/OpenGLCompat.h" ] && \
   [ -f "src/Cafe/HW/Latte/Renderer/OpenGL/OpenGLShim.h" ]; then
    echo -e "${GREEN}✓${NC} OpenGL compatibility headers exist"
else
    echo -e "${RED}✗${NC} OpenGL compatibility headers missing"
    ((errors++))
fi

# Check 6: Verify CMakeLists.txt has iOS detection
echo ""
echo "📋 Checking CMakeLists.txt for iOS support..."
if grep -q "IS_IOS" CMakeLists.txt; then
    echo -e "${GREEN}✓${NC} CMakeLists.txt has iOS detection"
else
    echo -e "${YELLOW}⚠${NC} CMakeLists.txt may be missing iOS detection"
    ((warnings++))
fi

# Check 7: Verify GitHub Actions workflow
echo ""
...
if $docs_ok; then
    echo -e "${GREEN}✓${NC} All key documentation files exist"
else
    ((warnings++))
fi
